...
function Sports(props) {
  ...
  const [sportsSourcesList, setSportsSourcesList] = useState([]);

  useEffect(() => {
    const findSources = async () => {
      const rowDataSports = await fetch(
        `https://newsapi.org/v2/top-headlines?country=fr&category=sports&apiKey=${NEWS_API_KEY}`
      );
      const dataSports = await rowDataSports.json();
      setSportsSourcesList(dataSports.articles);
    };
    findSources();
  }, []);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.scrowllView}>
        <View>
          ...
          {sportsSourcesList.map((article, i) => (
            <ListItem key={i}>
              <Avatar
