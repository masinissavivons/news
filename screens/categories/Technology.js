...
function Technology(props) {
  ...
  const [technologySourcesList, setTechnologySourcesList] = useState([]);

  useEffect(() => {
    const findSources = async () => {
      const rowDataTechnology = await fetch(
        `https://newsapi.org/v2/top-headlines?country=fr&category=technology&apiKey=${NEWS_API_KEY}`
      );
      const dataTechnology = await rowDataTechnology.json();
      setTechnologySourcesList(dataTechnology.articles);
    };
    findSources();
  }, []);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.scrowllView}>
        <View>
          ...
          {technologySourcesList.map((article, i) => (
            <ListItem key={i}>
              <Avatar
